...
VideoSourceNix::VideoSourceNix(int w, int h)
    : VideoSource(w, h)
{
    /* Setup video devices */
    int nd = 0;
    ...
    nsIOutputStream *pipe, nsIDOMCanvasRenderingContext2D *ctx)
{
    ...
    if (vidcap_src_capture_start(source, VideoSourceNix::Callback, this)) {
...
int
VideoSourceNix::Callback(
    vidcap_src *src, void *data, struct vidcap_capture_info *video)
{
    nsresult rv;
    PRUint32 wr;
    VideoSourceNix *vsm = static_cast<VideoSourceNix*>(data);
