...
AudioSourceDarwin::AudioSourceDarwin(int c, int r)
    : AudioSource(c, r)
{
    stream = NULL;
...
AudioSourceDarwin::~AudioSourceDarwin()
{
    Pa_Terminate();
}

nsresult
AudioSourceDarwin::Start(nsIOutputStream *pipe)
{
    PaError err;
    ...
    err = Pa_OpenStream(
        ...
        paClipOff, AudioSourceDarwin::Callback, this
    );
    if (err != paNoError) {
...
int
AudioSourceDarwin::Callback(const void *input, void *output,
    unsigned long frames, const PaStreamCallbackTimeInfo* timeInfo,
    PaStreamCallbackFlags statusFlags, void *data)
{
    nsresult rv;
    PRUint32 wr;
    AudioSourceDarwin *asa = static_cast<AudioSourceDarwin*>(data);
